...
A card's footer says **where its task lives**: the note's name, then every heading the task sits under (`Projects › Sprint 3 › Todo`). The chain comes from `utils/taskHeadings.ts` — `headingTrail` walks Obsidian's `metadataCache` headings backwards from the task's line, keeping each heading shallower than the last, which is the set of sections containing that line. The Tasks plugin's own `precedingHeader` is not used: it records only the nearest heading, and one level of a note is rarely where a task lives. A leading heading repeating the note's name (the `# H1` habit) is dropped, since it names the same place twice.
